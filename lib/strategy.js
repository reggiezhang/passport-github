...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    if (self._scope && self._scope.indexOf('user:email') !== -1) {
      var headers = { 'Authorization': self._oauth2.buildAuthHeader(accessToken), 'Accept': 'application/vnd.github.v3+json' };
      access_token= null;
      self._oauth2._request('GET', self._userProfileURL + '/emails', headers, '', accessToken, function(err, body, res) {
        if (err) {
